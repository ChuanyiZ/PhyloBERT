...
def main():
    ...
    model_dict: Dict[str, Task] = {
        ...
        "clinvar2_snv": Task(
            ...
            dataset=lambda: load_dataset(
                "/home/ac.chia/data/tiny/data_snv",
                data_files={"train": "train.tsv", "eval": "dev.tsv"}
            ),
            convert_func=convert_example_pairs_to_features,
            cast_func=cast_func,
        ...
        "clinvar2": Task(
            ...
            dataset=lambda: load_dataset(
                "/home/ac.chia/data/tiny/data_patho",
                data_files={"train": "train.tsv", "eval": "dev.tsv"}
            ),
            convert_func=convert_example_pairs_to_features,
            cast_func=cast_func,
    ...
    task_mapping = {task: i for i, task in enumerate(args.tasks)}
    def compute_metrics(eval_pred: EvalPrediction):
        predictions, labels, inputs = eval_pred
        task_names = inputs[1]
        results = {}
        for task in np.unique(task_names):
            idx = task_names == task_mapping[task]
    ...
    for task_name, dataset in dataset_dict.items():
        ...
        for phase, phase_dataset in dataset.items():
            features_dict[task_name][phase] = phase_dataset.map(
                ...
                cache_file_name=f"/home/ac.chia/data/cache/cache-{task_name}-{phase}-{phase_dataset._fingerprint}.arrow"
            )
