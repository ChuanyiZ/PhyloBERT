...
def main(args):
    ...
    for idx_project, project in enumerate(tqdm(TCGA_projects)):
        ...
        # remove duplicated
        embeddings = []
        for i in tqdm(range(200), leave=False):
            embeddings.append(np.load(f"{args.path}/TCGA_{project}.embedding_all.{i:03d}_of_200.npy", mmap_mode='r')[:, args.token, :])
        ...
        lde_filename = f".UMAP_LDE_token.project_{project}.token_{args.token}.model_{model}_{args.pooling_method}.npy"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--token", type=int, help="which token to use")
